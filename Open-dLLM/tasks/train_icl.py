...
from veomni.data.curriculum import Curriculum
from veomni.data.data_samplers import get_data_sampler
from veomni.data.task import get_task_sampler
...
def main():
    ...
    data_sampler = get_data_sampler(args.data.data_name, args.data.n_dims)
    task_sampler = get_task_sampler(args.train.task_name, args.data.n_dims, batch_size=512)
    curriculum = Curriculum(args.train.curriculum)
    train_dataset = build_task_dataset(
        train_steps=args.train.max_steps,
        batch_size=512,
        data_sampler=data_sampler,
        task_sampler=task_sampler,
        curriculum=curriculum,
        task_name=args.train.task_name,
    )
        ...
        for _ in range(start_step, args.train.train_steps):
            ...
            for micro_batch in micro_batches:
                ...
                with model_fwd_context:
                    ...
                    loss = output.loss
                    if loss is None:
                        raise ValueError("Model returned None loss. Check labels and loss computation.")
                    # loss is already a scalar, no need for .mean()
